  ...
  if (error || !appointment) {
    return (
      <div>
        ...
        <Link to="/appointments" className="text-blue-600 hover:text-blue-800 mt-4 inline-block">
        ...
        <div className="px-4 py-5 sm:px-6 flex justify-between items-center">
          <div>
            <h3 className="text-lg leading-6 font-medium text-gray-900">
              Appointment - {format(new Date(appointment.appointmentDate), 'MMMM dd, yyyy')}
            </h3>
            <p className="mt-1 max-w-2xl text-sm text-gray-500">Appointment Details</p>
          </div>
          {isAdmin && (
        ...
        <div className="border-t border-gray-200">
          <dl>
            <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              ...
              <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{appointment.appointmentId}</dd>
            </div>
            ...
            {appointment.patient && (
              <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                ...
                <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                  {appointment.patient.patientName} (ID: {appointment.patient.patientId})
                </dd>
              </div>
            )}
            {appointment.doctor && (
              <div className="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                ...
                <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                  {appointment.doctor.doctorName} - {appointment.doctor.speciality} (ID: {appointment.doctor.doctorId})
                </dd>
              </div>
            )}
            {appointment.nurse && (
              <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                ...
                <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                  {appointment.nurse.nurseName} (ID: {appointment.nurse.nurseId})
                </dd>
              </div>
            )}
          </dl>
